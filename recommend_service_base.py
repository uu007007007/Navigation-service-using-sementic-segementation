...
register_coco_instances("val_dataset", {}, '/home/road2022/parking_project/detectron2/base_val_random.json', '/home/road2022/parking_project/detectron2/image')
...
cfg.MODEL.WEIGHTS = "/home/road2022/parking_project/detectron2/base_model.pth"  # 모델 경로
...
output_video_path = 'service_base.mov'
	...
	def main(self, mode):
		...
		# 비디오 처리
			...
			while True:
				...
				num_parking = np.sum(pred_classes.numpy() == 1)
				print(num_parking)
				...
				cv2.putText(out_image, f"FPS: {FPS}", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
				cv2.putText(out_image, f"Num Parking: {num_parking}", (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
				out.write(out_image)
				# 결과 출력
		...
		for i in range(len(pred_classes)):
			...
			if class_id == 1:
				...
				if len(y) > 0 and len(x) > 0:
					y, x = y[0]-10, x[0]  # 첫 번째 마스크 위치
					cv2.putText(
						im, f"{class_name}: {score:.2f}, Dis: {distance:.2f}", 
						(x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1
					)
			else:
				if len(y) > 0 and len(x) > 0:
					y, x = y[0]-10, x[0]  # 첫 번째 마스크 위치
